...
import { rapportsContent } from "@/lib/content/politique-monetaire/rapports-content";
import { localizeArticle } from "@/lib/theme-localize";
import { ThemeArticle } from "@/components/public/content/theme-page";

const PAGE_PATH = "/politique-monetaire/publications/rapports";
const PAGE_NAMESPACE = "pages.politiqueMonetaire.sub.rapports";
...
export default async function RapportsPage({ params }: PageProps) {
  const { locale: rawLocale } = await params;
  const locale = resolveLocaleParam(rawLocale);
  ...
  return (
    <ThemeArticle
      content={localizeArticle(rapportsContent, t, tRelated)}
