...
import { facilitesContent } from "@/lib/content/politique-monetaire/facilites-content";
import { localizeArticle } from "@/lib/theme-localize";
import { ThemeArticle } from "@/components/public/content/theme-page";

const PAGE_PATH = "/politique-monetaire/taux-directeurs/facilites";
const PAGE_NAMESPACE = "pages.politiqueMonetaire.sub.facilites";
...
export default async function FacilitesPage({ params }: PageProps) {
  const { locale: rawLocale } = await params;
  const locale = resolveLocaleParam(rawLocale);
  ...
  return (
    <ThemeArticle
      content={localizeArticle(facilitesContent, t, tRelated)}
