...
/**
 * Localization layer of the Monnaie theme pages.
 *
 * Every display string of the theme lives in the message catalogs
 * (`apps/messages/{fr,en}.json`) under `pages.monnaie.<page>` (per page) and
 * `pages.monnaie.related` (shared cross-links). The content files hold the
 * *structure* only: section ids, message keys, hrefs, icons and pure data
 * (figures, values, years).
 *
 ...
 * assembles the strings provided by the active locale's catalog.
 */
...
export type ArticleSectionConfig = {
  /** Message path segment under `pages.monnaie.<page>.sections`. */
  key: string;
...
export type ArticleContent = {
  ...
  /** Shared cross-links: `key` resolves `pages.monnaie.related.<key>.*`, href is structural. */
 ...
 *
 * @param t         translator of the page (`pages.monnaie.<page>`).
 * @param tRelated  translator of the shared cross-links (`pages.monnaie.related`).
 */
