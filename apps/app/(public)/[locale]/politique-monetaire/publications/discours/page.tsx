...
import { discoursContent } from "@/lib/content/politique-monetaire/discours-content";
import { localizeArticle } from "@/lib/theme-localize";
import { ThemeArticle } from "@/components/public/content/theme-page";

const PAGE_PATH = "/politique-monetaire/publications/discours";
const PAGE_NAMESPACE = "pages.politiqueMonetaire.sub.discours";
...
export default async function DiscoursPage({ params }: PageProps) {
  const { locale: rawLocale } = await params;
  const locale = resolveLocaleParam(rawLocale);
  ...
  return (
    <ThemeArticle
      content={localizeArticle(discoursContent, t, tRelated)}
